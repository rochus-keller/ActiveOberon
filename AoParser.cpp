...
static inline bool FIRST_ProcHead(int tt) {
	return tt == Tok_Lbrack || tt == Tok_Star || tt == Tok_Amp || tt == Tok_ident || tt == Tok_Minus;
}
...
static inline bool FIRST_Body(int tt) {
	return tt == Tok_BEGIN || tt == Tok_END;
}
...
static inline bool FIRST_MulOp(int tt) {
	return tt == Tok_Slash || tt == Tok_Star || tt == Tok_Amp || tt == Tok_DIV || tt == Tok_MOD;
}
...
void Parser::Module(SynTree* st) {
	...
	while( FIRST_Definition(la.d_type) ) {
		Definition(st);
	}
	while( FIRST_DeclSeq(la.d_type) ) {
		DeclSeq(st);
	}
	Body(st);
	if( expect(Tok_ident, false, "Module") ) addTerminal(st);
...
void Parser::ImportList(SynTree* st) {
	...
	if( expect(Tok_ident, false, "ImportList") ) addTerminal(st);
	if( la.d_type == Tok_ColonEq ) {
		if( expect(Tok_ColonEq, false, "ImportList") ) addTerminal(st);
		if( expect(Tok_ident, false, "ImportList") ) addTerminal(st);
	}
...
void Parser::Definition(SynTree* st) {
	...
	if( la.d_type == Tok_REFINES ) {
		if( expect(Tok_REFINES, false, "Definition") ) addTerminal(st);
		Qualident(st);
	}
	while( la.d_type == Tok_PROCEDURE ) {
		if( expect(Tok_PROCEDURE, false, "Definition") ) addTerminal(st);
		if( expect(Tok_ident, false, "Definition") ) addTerminal(st);
...
void Parser::DeclSeq(SynTree* st) {
	...
	} else if( FIRST_ProcDecl(la.d_type) || la.d_type == Tok_TYPE || la.d_type == Tok_PROCEDURE || la.d_type == Tok_CODE || la.d_type == Tok_END || la.d_type == Tok_VAR || la.d_type == Tok_BEGIN || la.d_type == Tok_CONST ) {
...
void Parser::Assembler(SynTree* st) {
	...
	if( expect(Tok_END, false, "Assembler") ) addTerminal(st);
}
...
void Parser::ObjectType(SynTree* st) {
	...
	if( FIRST_SysFlag(la.d_type) || la.d_type == Tok_Lpar || la.d_type == Tok_IMPLEMENTS || FIRST_DeclSeq(la.d_type) || FIRST_Body(la.d_type) ) {
		if( FIRST_SysFlag(la.d_type) ) {
			SysFlag(st);
		}
		if( la.d_type == Tok_Lpar ) {
		...
		if( la.d_type == Tok_IMPLEMENTS ) {
			if( expect(Tok_IMPLEMENTS, false, "ObjectType") ) addTerminal(st);
			Qualident(st);
		}
		while( FIRST_DeclSeq(la.d_type) ) {
			DeclSeq(st);
...
void Parser::Body(SynTree* st) {
	...
	if( FIRST_StatBlock(la.d_type) ) {
		StatBlock(st);
	} else if( la.d_type == Tok_END ) {
		if( expect(Tok_END, false, "Body") ) addTerminal(st);
	} else
		invalid("Body");
}
...
void Parser::StatBlock(SynTree* st) {
	...
	if( expect(Tok_END, false, "StatBlock") ) addTerminal(st);
}
...
void Parser::AssigOrCall(SynTree* st) {
	...
	Designator(st);
	if( la.d_type == Tok_ColonEq || la.d_type == Tok_Lpar ) {
		if( la.d_type == Tok_ColonEq ) {
			if( expect(Tok_ColonEq, false, "AssigOrCall") ) addTerminal(st);
			Expr(st);
		} else if( la.d_type == Tok_Lpar ) {
			if( expect(Tok_Lpar, false, "AssigOrCall") ) addTerminal(st);
			ExprList(st);
			if( expect(Tok_Rpar, false, "AssigOrCall") ) addTerminal(st);
		} else
			invalid("AssigOrCall");
	}
}
...
void Parser::Term(SynTree* st) {
	...
	if( la.d_type == Tok_Plus || la.d_type == Tok_Minus ) {
		if( la.d_type == Tok_Plus ) {
			if( expect(Tok_Plus, false, "Term") ) addTerminal(st);
		} else if( la.d_type == Tok_Minus ) {
			if( expect(Tok_Minus, false, "Term") ) addTerminal(st);
		} else
			invalid("Term");
	}
	Factor(st);
...
void Parser::Designator(SynTree* st) {
	...
	while( ( peek(1).d_type == Tok_Dot && peek(2).d_type == Tok_ident )  || la.d_type == Tok_Lbrack || la.d_type == Tok_Hat || la.d_type == Tok_Lpar ) {
		if( ( peek(1).d_type == Tok_Dot && peek(2).d_type == Tok_ident )  ) {
			if( expect(Tok_Dot, false, "Designator") ) addTerminal(st);
			if( expect(Tok_ident, false, "Designator") ) addTerminal(st);
		} else if( la.d_type == Tok_Lbrack ) {
			if( expect(Tok_Lbrack, false, "Designator") ) addTerminal(st);
			ExprList(st);
			if( expect(Tok_Rbrack, false, "Designator") ) addTerminal(st);
		} else if( la.d_type == Tok_Hat ) {
			if( expect(Tok_Hat, false, "Designator") ) addTerminal(st);
		} else if( la.d_type == Tok_Lpar ) {
			if( expect(Tok_Lpar, false, "Designator") ) addTerminal(st);
			if( FIRST_ExprList(la.d_type) ) {
				ExprList(st);
			}
			if( expect(Tok_Rpar, false, "Designator") ) addTerminal(st);
		} else
			invalid("Designator");
	}
}
