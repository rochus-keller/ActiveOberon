...
OberonPainter2::OberonPainter2(QObject* parent) :
...
void OberonPainter2::addBuiltIn(const QByteArray& bi)
{
    d_builtins << bi;
}

void OberonPainter2::addKeyword(const QByteArray& kw)
{
    d_keywords << kw;
}

void OberonPainter2::addBuiltIns()
{
    for(int i = Ast::Type::NIL; i < Ast::Type::MaxBasicType; i++ )
        d_builtins << Ast::Type::name[i];
    for(int i = Ast::Builtin::ABS; i <= Ast::Builtin::SYSTEM; i++ )
        d_builtins << Ast::Builtin::name[i];

    d_builtins << "TRUE" << "FALSE";
}

QTextCharFormat OberonPainter2::formatForCategory(int c) const
{
    return d_format[c];
}

void OberonPainter2::highlightBlock(const QString& text)
{
